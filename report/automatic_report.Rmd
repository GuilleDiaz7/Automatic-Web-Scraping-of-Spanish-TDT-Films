...
So, if there are `r nrow(df)` instances, why do we only have `r n_distinct(df$sp_title)` movies? I guess some of them were broadcasted more than once. These is the top 10 (I filtered a couple of instances that are not films, but regular TV programs):
...
A boxplot may be easier to interpret. It is remarkable how similar are lengths in Antena 3 channel.
